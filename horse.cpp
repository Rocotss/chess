...
bool Horse::stroke(Step step, Figure* board[height][width])
{
    bool ans=false;

    if(((abs(step.h-step.last_h)==2 && abs(step.w-step.last_w)==1) ||
            (abs(step.h-step.last_h)==1 && abs(step.w-step.last_w)==2)) &&
